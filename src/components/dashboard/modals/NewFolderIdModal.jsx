...
import { createNewIdFolder } from "@/actions/mutations/folder";
...
function NewFolderIdModal({
  ...
  const newFolderMutation = useMutation({
    mutationFn: async (foldername) => {
      setIsLoading(true);
      const data = await createNewIdFolder(foldername);
      setIsLoading(false);
...
export default NewFolderIdModal;
